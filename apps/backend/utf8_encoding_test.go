...
// TestUTF8Roles проверяет, что все роли с русскими названиями корректны
func TestUTF8Roles(t *testing.T) {
	db, err := sql.Open("pgx", "postgres://complisec:complisec123@localhost:5432/complisec?sslmode=disable")
	if err != nil {
		t.Fatalf("Ошибка подключения к БД: %v", err)
	}
	...
	rows, err := db.Query("SELECT name, description FROM roles WHERE description IS NOT NULL")
	if err != nil {
		t.Fatalf("Ошибка запроса ролей: %v", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&name, &description); err != nil {
			t.Errorf("Ошибка сканирования роли %s: %v", name, err)
			continue
		}

		// Проверяем название роли
		if !utf8.ValidString(name) {
			t.Errorf("Роль %s имеет некорректное UTF-8 название: %q", name, name)
		}

		// Проверяем описание роли
		if !utf8.ValidString(description) {
			t.Errorf("Роль %s имеет некорректное UTF-8 описание: %q", name, description)
		}

		// Проверяем на mojibake
		if containsMojibake(name) || containsMojibake(description) {
			t.Errorf("Роль %s содержит mojibake: название=%q, описание=%q", name, name, description)
		}
	}
}
